...
void loop() {
  if (Serial.available() >= 5) {
    ...
    float val = 0;
    Serial.readBytes((uint8_t*)&val, 4);
    //ici j'ai changé j'ai plus un octet mais un double
    //je dois modifier la ligne au-dessus pour la concordance des types
    // On effectue la commande associé
    if (ordre == AngleOrder) {
      float pwm_angle = val*185+1365; //on donne entre -1 et 1 sur python: -1 tourne au max à gauche et 1 à droite
      //vérifier si les concordances des types est correcte
      Serial.print("angle: ");
      Serial.println(pwm_angle);
      // On écrit la valeur sur le servo
      angle.writeMicroseconds(round(pwm_angle));
    } else if (ordre == SpeedOrder) {
      float pwm_vitesse = val*200+1500;  //on donne entre -1 et 1, à partir de 0 çca avance, pas de marche arrière pour l'instant
      Serial.print("speed: ");
      Serial.println(pwm_vitesse);
      // On écrit sur l'ESC
      esc.writeMicroseconds(round(pwm_vitesse)); 
    }
    /*
